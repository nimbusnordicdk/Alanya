...
class CHBSPlugin
{
	...
	{		
		...
		/* License */
		if($License->verify($option['license_username'],$option['license_purchase_code']))
		{
			$License->setAsVerified();
		}
		else
		{
			$License->setAsUnVerified();
			$Notice->addError(CHBSHelper::getFormName('license_purchase_code',false),__('Purchase code is not valid or the purchase code belongs to the different username or product.','chauffeur-booking-system'));				
		}
	...
	function adminNotice()
	{
		...
		if(!$License->isVerified(CHBSOption::getOption('license_username'),CHBSOption::getOption('license_purchase_code')))
		{
			echo 
			'
				<div class="notice notice-error">
