...
class CHBSGeoLocation
{
    ...
    function getDocument()
    {
        if (CHBSGlobalData::isSetGlobalData('geolocation_document')) {
            ...
            // Set the SSL context to use TLSv1.2
            $context = stream_context_create(array(
                'http' => array(
                    'timeout' => 3,
                    'user_agent' => 'keycdn-tools:' . $pageURL['scheme'] . '://' . $pageURL['host']
                ),
                'ssl' => array(
                    'verify_peer' => true,
                    'verify_peer_name' => true,
                    'allow_self_signed' => false,
                    'crypto_method' => STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT  // Force TLSv1.2
                )
            ));

            // Attempt to get the content using the updated context
            if (($document = @file_get_contents($url, false, $context)) === false) {
                return false; // Return false if the content can't be fetched
            }
        }
        ...
        if ((is_object($document)) && ($document instanceof stdClass)) {
            // Handle the document object if needed
        } else {
